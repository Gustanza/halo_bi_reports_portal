...
const handleSubmit = () => {
  ...
  if (!email || !token) {
    error.value = "Missing reset link information. Please request a new link.";
    return;
  }
  ...
  axiosClient
    ...
    .then(() => {
      status.value = "Password reset successfully. Redirecting to login...";
      setTimeout(() => router.push({ name: "Login" }), 1500);
    })
    .catch(() => {
      error.value = "Could not reset password. Please try again.";
    })
    .finally(() => {
      loading.value = false;
    });
